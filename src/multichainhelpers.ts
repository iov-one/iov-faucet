...
export function identitiesOfFirstChain(profile: UserProfile): ReadonlyArray<PublicIdentity> {
...
export async function accountsOfFirstChain(
  ...
  const addresses = identitiesOfFirstChain(profile).map(identity => signer.identityToAddress(identity));
...
export async function refillFirstChain(profile: UserProfile, signer: MultiChainSigner): Promise<void> {
  ...
  const holderIdentity = identitiesOfFirstChain(profile)[0];
